...
#..###
####!!
`.trim(),f=g.split("\n").map(t=>t.trim()),w=f.length,y=f[0].length;console.assert(f.every(t=>t.length==y),"Bad ascii level");const m=f.map(t=>t.split("").map(t=>"#"==t)).flat();function x(t,e){return!(t<0)&&!(t>=y)&&!(e<0)&&!(e>=w)&&m[t+e*y]}let v=f.map(t=>t.split("").map(t=>"#"!=t&&"!"!=t)).flat(),b={debug_x:0,debug_y:0,player_pos:new h(2,3),crates_pos:[new h(1,3),new h(3,4)]};class A{constructor(t,e){this.original_pos=t,this.dir=e}execute(){h.add(this.original_pos,this.dir,b.player_pos)}animTurn(t){h.add(this.original_pos,h.scale(this.dir,t),T.player_sprite.position)}undoAnimTurn(t){this.animTurn(1-t)}undo(){h.copy(this.original_pos,b.player_pos)}}class R{constructor(t,e,i){this.crate_index=t,this.original_pos=e,this.dir=i,this.extra_command=new A(h.sub(e,i),i)}execute(){h.add(this.original_pos,this.dir,b.crates_pos[this.crate_index]),this.extra_command.execute()}animTurn(t){h.add(this.original_pos,h.scale(this.dir,t),T.crates_sprites[this.crate_index].position),this.extra_command.animTurn(t)}undoAnimTurn(t){var e;this.extra_command.animTurn(1-t);let i=h.add(this.original_pos,new h(.5,.5));t<.5?(h.scale(h.one,1-2*t,T.crates_sprites[this.crate_index].size),h.add(i,this.dir,i)):(h.scale(h.one,2*t-1,T.crates_sprites[this.crate_index].size),h.copy(this.original_pos,T.crates_sprites[this.crate_index].position)),e=T.crates_sprites[this.crate_index],h.sub(i,h.scale(e.size,.5),e.position)}undo(){h.copy(this.original_pos,b.crates_pos[this.crate_index]),this.extra_command.undo()}}class E{constructor(t,e){this.pos=t,this.dir=e}execute(){throw Error("not an executable command")}animTurn(t){h.add(this.pos,h.scale(this.dir,t*(1-t)),T.player_sprite.position)}undoAnimTurn(t){throw Error("not an undoable command")}undo(){throw Error("not an undoable command")}}let q=[],T={turn_time:0,player_sprite:B(h.copy(b.player_pos),h.copy(h.one),h.copy(h.zero),h.copy(h.one)),crates_sprites:b.crates_pos.map(t=>B(h.copy(t),h.copy(h.one),h.copy(h.zero),h.copy(h.one)))},N=null,F=null;function B(t,e,i,r){return{buffer_index:0,position:t,size:e,uv_pos:i,uv_size:r}}let D={pressed:{},queued:[]};requestAnimationFrame(function t(e){let i=(e-C)*.001;if(C=e,o.resizeCanvasToDisplaySize(p.canvas)&&p.viewport(0,0,p.drawingBufferWidth,p.drawingBufferHeight),D.queued.length>0&&null===N&&null===F){let t=D.queued.shift();if("KeyZ"==t)q.length>0&&((F=q.pop()).undo(),T.turn_time=0);else{let e=new h;switch(t){case"KeyD":e.x=1;break;case"KeyA":e.x=-1;break;case"KeyW":e.y=-1;break;case"KeyS":e.y=1}if(0!=e.x||0!=e.y){let t=h.add(b.player_pos,e);if(x(t.x,t.y))N=new E(b.player_pos,e);else{let i=b.crates_pos.findIndex(e=>h.equals(t,e));if(-1==i)(N=new A(h.copy(b.player_pos),e)).execute(),q.push(N);else{let t=h.add(b.crates_pos[i],e);x(t.x,t.y)||b.crates_pos.some(e=>h.equals(t,e))?N=new E(b.player_pos,e):((N=new R(i,h.copy(b.crates_pos[i]),e)).execute(),q.push(N))}}}}}null!==N?(T.turn_time+=i/.05,T.turn_time=n(T.turn_time,0,1),N.animTurn(T.turn_time),T.turn_time>=1&&(N=null,T.turn_time=0)):null!==F&&(T.turn_time+=i/.05,T.turn_time=n(T.turn_time,0,1),F.undoAnimTurn(T.turn_time),T.turn_time>=1&&(F=null,T.turn_time=0)),p.clear(p.COLOR_BUFFER_BIT),_.centerLevel(y,w,64);for(let t=0;t<w;t++)for(let e=0;e<y;e++)v[e+t*y]?_.queue("Floor",e,t):m[e+t*y]&&_.queue("Wall",e,t);_.queueExtra("Player",T.player_sprite.position,T.player_sprite.size),T.crates_sprites.forEach(t=>{_.queueExtra("Crate",t.position,t.size)}),_.draw(),requestAnimationFrame(t)}),document.addEventListener("keydown",function(t){console.log("keydown"),D.pressed[t.code]=!0,D.queued.push(t.code)}),document.addEventListener("keyup",function(t){console.log("keyup"),D.pressed[t.code]=!1});let C=0;
//# sourceMappingURL=index.f5419176.js.map
